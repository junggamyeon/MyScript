...
local SAFE_COORD = Vector3.new(0, 104.51000213623047, 0)
...
local RUNNING      = true
local POST_RADIO   = false      -- đã tele radio + chuyển sang spam ở SAFE_COORD
...
local function remoteAttack(mon)
    ...
    if mon then
        ...
        -- Nếu không truyền mục tiêu, vẫn ping skill tấn công tầm xa (fallback)
...
-- === AUTO PROXIMITY PROMPT (map objects only) ===
    ...
    while true do
        if RUNNING then
            if not POST_RADIO then
                ...
                -- POST_RADIO: đứng SAFE_COORD, spam attack; nếu có CHOPPER thì bám theo bằng teleport liên tục
                local ch = findChopperPart()
                if ch then
                    stickToChopper()
                    remoteAttack(nil) -- ping đòn tầm xa nếu có
                    task.wait(0.05)
                else
                    local h = safeHRP()
                    ...
                    -- tấn công liên tục nhưng không tele tới quái
                    local mon = nearestEntity()
                    remoteAttack(mon) -- nếu có mục tiêu gần thì dùng nó, nếu nil sẽ dùng fallback
                end
            end
        end
