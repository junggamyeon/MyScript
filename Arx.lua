...
local settingsRoot    = "animerangerx"
local productFolder   = "Shouko.Dev - Anime Ranger X"
local perAccountDir   = settingsRoot .. "/" .. productFolder
local fileName        = (LocalPlayer and LocalPlayer.Name or "player") .. "_settings.json"

if not isfolder(settingsRoot) then makefolder(settingsRoot) end
if not isfolder(perAccountDir) then makefolder(perAccountDir) end
...
local function loadSettings()
    local full = perAccountDir .. "/" .. fileName
    if isfile(full) then
        return HttpService:JSONDecode(readfile(full))
    else
        writefile(full, HttpService:JSONEncode(defaultSettings))
        return table.clone(defaultSettings)
    end
end

local function saveSettings(tbl)
    writefile(perAccountDir .. "/" .. fileName, HttpService:JSONEncode(tbl))
end
...
statsSection:Toggle({
    ...
    function autoRollTrail(unitEntry, desiredTrails)
        ...
        print("🔁 Start roll trail cho:", unitName)
...
trailRerollSection:Toggle({
    ...
    Callback = function(val)
        ...
        if val then
            print("🔁 Start roll trail:", rerollConfig.unit, "với trail:", rerollConfig.trail)
