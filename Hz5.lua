...
-- Bật nếu game có remote + buffer (như script bạn gửi)
local ENABLE_REMOTE_ATTACK = true
local ENABLE_REMOTE_SKILLS = true
local SKILL_INTERVAL = 1 -- giây, nhịp spam skill
...
if not Doors then error("[Flow] Không có workspace.School.Doors/Door") end

local Entities = workspace:FindFirstChild("Entities")
if not Entities then error("[Flow] Không có workspace.Entities") end

-- Remote (giống file bạn gửi)
...
local function remoteAttack(mon)
    ...
    pcall(function()
        Remote:FireServer(buf("\a\004\001"), {h.Position.Magnitude})
    end)
end

local function startSkills()
    if skillsOn or not (ENABLE_REMOTE_SKILLS and Remote and HAS_BUFFER) then return end
    skillsOn = true
    task.spawn(function()
        while skillsOn do
            -- payload như script gốc của bạn
            pcall(function() Remote:FireServer(buf("\a\003\001"), {tick()}) end) -- Z
            pcall(function() Remote:FireServer(buf("\a\006\001"), {tick()}) end) -- C
            -- thêm skill khác nếu muốn:
            -- pcall(function() Remote:FireServer(buf("\a\005\001"), {tick()}) end) -- X
            -- pcall(function() Remote:FireServer(buf("\a\a\001"), {tick()}) end)   -- G
            pcall(function() Remote:FireServer(buf("\v")) end)                    -- E
            task.wait(SKILL_INTERVAL)
        end
    end)
end
local function stopSkills() skillsOn=false end
...
local function tweenDoorsOnce()
    ...
    if #doors==0 then warn("[Flow] Doors rỗng/không có PrimaryPart"); return end

    runningTween   = true
    pauseFarm      = true

    disableVelocityLock()
    _hrp.Anchored  = true  -- GIỮ ĐỘ CAO: nhân vật sẽ không rơi trong suốt quá trình tween
    ...
    for i,entry in ipairs(doors) do
        ...
        if d and d.PrimaryPart then
            ...
            if i==#doors then
                ...
                task.wait(DOOR_PAUSE) -- vẫn Anchor nên KHÔNG rơi
            end
        end
    end

    _hrp.Anchored = false   -- cho phép di chuyển để farm
    enableVelocityLock()
...
-- ================== ENTITY WATCHER ===================
-- Quy tắc:
--  - Entities trống -> awaitingWave=true (chờ map spawn mới)
--  - Entities vừa có model mới (và awaitingWave=true) -> TWEEN CỬA trước, sau đó mới farm
awaitingWave = (countEntities()==0)

Entities.ChildRemoved:Connect(function()
    if countEntities()==0 then
        awaitingWave = true
        stopSkills()           -- dừng skill khi hết quái
    end
end)

Entities.ChildAdded:Connect(function()
    if awaitingWave and not runningTween then
        tweenDoorsOnce()       -- bắt buộc tween cửa trước
        awaitingWave = false
        if ENABLE_REMOTE_SKILLS then startSkills() end  -- bật skill khi bắt đầu farm wave mới
    end
end)

-- ===================== MAIN =====================
-- Khởi động: nếu đang có quái -> tween cửa rồi farm; nếu chưa -> chờ wave mới
if countEntities() > 0 then
    tweenDoorsOnce()
    awaitingWave = false
    if ENABLE_REMOTE_SKILLS then startSkills() end
else
    awaitingWave = true
end
...
plr.CharacterAdded:Connect(function(c)
    ...
    enableVelocityLock()
    if ENABLE_REMOTE_SKILLS and not awaitingWave then startSkills() end
end)
