...
local FieldsFolder = workspace:WaitForChild("Zones"):WaitForChild("Fields")
...
local function fieldCenterPos(fieldObj)
	...
	if fieldObj:IsA("BasePart") then
		return fieldObj.Position + Vector3.new(0, 6, 0)
	end
	if fieldObj:IsA("Model") then
		local cf = fieldObj:GetBoundingBox()
		return cf.Position + Vector3.new(0, 6, 0)
	end
	return nil
end
...
local function fieldList()
	...
	for _, f in ipairs(FieldsFolder:GetChildren()) do
		if f:IsA("BasePart") or f:IsA("Model") then
