...
local function moveStep(pos, speed)
	...
	if State.ActiveTween == tw then
		State.ActiveTween = nil
	end

	local _, _, hrp2 = getChar()
	if (hrp2.Position - pos).Magnitude > math.max(6, d * 0.65) then
		pcall(function()
...
-- FIX: tìm đúng platform tổ của bạn (Owner có thể là Player instance hoặc string)
local function getMyAnthillPlatform()
	for _, a in ipairs(AnthillsFolder:GetChildren()) do
		...
		if owner and plat and plat:IsA("BasePart") then
			local v = owner.Value
...
-- Tele/tween về platform đúng tổ (kèm fallback CFrame nếu anti-tween)
local function ensureTeleToPlatform(plat)
	if not (plat and plat.Parent and plat:IsA("BasePart")) then return false end
	...
	for _ = 1, 80 do
		...
		local after = hrp.Position
		if (after - before).Magnitude < 0.25 then
			pcall(function()
...
-- AUTO CONVERT (FIX): đúng tổ của bạn + không treo + nếu không tìm thấy platform thì nhả converting để farm không đứng
task.spawn(function()
...
Right:CreateLabel({ Name = "Status: Ready" }, "STATUS_LBL")
