...
local function moveStep(pos, speed)
	...
	local t = math.min(0.25, d / (speed or 120))
	...
	while not ok and os.clock() - start < (t + 0.2) do
...
local function calcDominantY(container)
	...
	return sampleY and math.floor(sampleY + 0.5) or nil
end
...
local function buildRoamPoints()
	...
	local rx = math.max(8, (maxv.X - minv.X) * 0.5)
	local rz = math.max(8, (maxv.Z - minv.Z) * 0.5)

	local pts = {}
	local n = 8
	for i = 1, n do
		local a = (i - 1) * (math.pi * 2 / n)
		local x = cx + math.cos(a) * rx * 0.9
		local z = cz + math.sin(a) * rz * 0.9
	...
	while task.wait(0.2) do
		if State.AutoConvert and not State.Converting then
			...
			if m > 0 and c >= m then
				...
				if pos then
					setFlyStable()
					while State.AutoConvert and State.Converting and (lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")) do
						if not pos then break end
						moveStep(pos, 150)
						local cc, mm = getPollen()
						if mm == 0 or cc <= 0 then break end
						task.wait(0.05)
					end
				end
	...
	while task.wait(0.05) do
		...
		if State.SelectedToken then
			while State.AutoFarm and not State.Converting do
				...
				moveStep(withFarmY(State.SelectedToken.Position + Vector3.new(0, 2, 0)), 170)
				task.wait(0.02)
			end

			if State.LastRoamPos and State.AutoFarm and not State.Converting then
				moveStep(withFarmY(State.LastRoamPos), 150)
			end
