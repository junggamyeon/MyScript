...
local SAFE_COORD = Vector3.new(77.211, -10.755, -90.021)
...
local RUNNING = true
local POST_RADIO = false
local AT_SAFE_POS = false
local FOLLOW_CHOPPER = false
...
local function remoteAttack(mon)
    if not (ENABLE_REMOTE_ATTACK and Remote and HAS_BUFFER) then return end
    if mon then
        ...
        pcall(function() Remote:FireServer(buf("\v")) end)
    end
end
...
local function collectDropItems()
    ...
    if POST_RADIO then
        _hrp.CFrame = CFrame.new(SAFE_COORD)
        AT_SAFE_POS = true
    end
end
...
local function findJarstRadioPart()
    local school = workspace:FindFirstChild("School"); if not school then return nil end
    local rooms = school:FindFirstChild("Rooms"); if not rooms then return nil end
    local roof = rooms:FindFirstChild("RooftopBoss"); if not roof then return nil end
    local stat = roof:FindFirstChild("StaticProps"); if not stat then return nil end
    local target = stat:FindFirstChild("jarst_radio"); if not target then return nil end
...
local function findChopperPart()
    local school = workspace:FindFirstChild("School"); if not school then return nil end
    local rooms = school:FindFirstChild("Rooms"); if not rooms then return nil end
    local roof = rooms:FindFirstChild("RooftopBoss"); if not roof then return nil end
    local target = roof:FindFirstChild("Chopper"); if not target then return nil end
    if target:IsA("BasePart") then return target end
    if target:IsA("Model") then
        if target.PrimaryPart then return target.PrimaryPart end
        for _, d in ipairs(target:GetDescendants()) do
...
local function getObjectiveLabel()
    local pg = plr:FindFirstChild("PlayerGui"); if not pg then return nil end
    local ms = pg:FindFirstChild("MainScreen"); if not ms then return nil end
    local od = ms:FindFirstChild("ObjectiveDisplay"); if not od then return nil end
    local oe = od:FindFirstChild("ObjectiveElement"); if not oe then return nil end
    local list = oe:FindFirstChild("List"); if not list then return nil end
    local desc = list:FindFirstChild("Description")
    return desc
end

local function objectiveHasEscape()
    local lb = getObjectiveLabel(); if not lb then return false end
    local t = tostring(lb.Text or ""):lower()
    return t:find("escape") ~= nil
end

local function goNearRadioThenSafe()
    local _hrp = safeHRP(); if not _hrp then return end
    local part = findJarstRadioPart(); if not part then return end
    _hrp.CFrame = part.CFrame + Vector3.new(0, 2, 0)
    local t0 = os.clock()
...
local function stickToChopper()
    local _hrp = safeHRP(); if not _hrp then return end
    local ch = findChopperPart(); if not ch then return end
    local targetPos = ch.Position + Vector3.new(0, ABOVE_OFFSET, 0)
    _hrp.CFrame = CFrame.new(targetPos, ch.Position)
end

local function spamAtSafeCoord()
    local mon = nearestEntity()
    if mon then remoteAttack(mon) else remoteAttack(nil) end
end
...
task.spawn(function()
    while true do
        if RUNNING then
            if not POST_RADIO then
                if countEntities() == 0 then
                    local radio = findJarstRadioPart()
                    if radio then
                        goNearRadioThenSafe()
                        spamAtSafeCoord()
                    else
                ...
                if not FOLLOW_CHOPPER and objectiveHasEscape() then
                    FOLLOW_CHOPPER = true
                end
                if FOLLOW_CHOPPER then
                    stickToChopper()
                    remoteAttack(nil)
                    task.wait(0.05)
                else
                    local h = safeHRP()
                    if not AT_SAFE_POS or (h.Position - SAFE_COORD).Magnitude > 3 then
                        h.CFrame = CFrame.new(SAFE_COORD)
                        AT_SAFE_POS = true
                        spamAtSafeCoord()
                    else
                        spamAtSafeCoord()
...
plr.CharacterAdded:Connect(function(c)
    ...
    if RUNNING then enableVelocityLock() end
    if POST_RADIO then
        hrp.CFrame = CFrame.new(SAFE_COORD)
        AT_SAFE_POS = true
    end
end)
