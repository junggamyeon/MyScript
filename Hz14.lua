...
local RUNNING      = true
local RADIO_MODE   = false    -- khi true: đã phát hiện jarst_radio và đã xử lý chuyển vị trí
...
local function collectDropItems()
    ...
    for _, part in ipairs(DropItems:GetChildren()) do
        if not RUNNING or RADIO_MODE then return end
...
local function findJarstRadioPart()
    local ok, school = pcall(function() return workspace:FindFirstChild("School") end)
    if not ok or not school then return nil end
...
local function moveToRadioAndHover()
    local _hrp = safeHRP(); if not _hrp then return end
    local part = findJarstRadioPart(); if not part then return end
    _hrp.CFrame = part.CFrame + Vector3.new(0, 5, 0)
    task.wait(0.1)
    _hrp.CFrame = _hrp.CFrame + Vector3.new(0, 10, 0)
    _hrp.Anchored = true
end
...
task.spawn(function()
    while true do
        if RUNNING and not RADIO_MODE then
...
task.spawn(function()
    while true do
        if not RADIO_MODE then
            local part = findJarstRadioPart()
            if part then
                RUNNING = false
                stopAllMotion()
                RADIO_MODE = true
