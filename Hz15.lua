...
local function goNearRadioWaitThenHover()
    local _hrp = safeHRP(); if not _hrp then return end
    local part = findJarstRadioPart(); if not part then return end
    _hrp.CFrame = part.CFrame + Vector3.new(0, 2, 0) -- đứng gần radio để autoprompt chạy
    local t0 = os.clock()
    while os.clock() - t0 < 3 do
        autoPromptNearby()
        task.wait(0.05)
    end
    _hrp.CFrame = _hrp.CFrame + Vector3.new(0, 15, 0)
    _hrp.Anchored = true
    RUNNING = false
    disableVelocityLock()
end
...
task.spawn(function()
    while true do
        if RUNNING then collectDropItems() end
        task.wait(0.3)
    end
end)

-- autoprompt loop (vẫn chạy bình thường khi RUNNING)
task.spawn(function()
    while true do
        if RUNNING then autoPromptNearby() end
        task.wait(0.25)
    end
end)
